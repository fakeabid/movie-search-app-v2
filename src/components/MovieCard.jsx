export default function MovieCard(props) {
    ...
    function editWatchList(e) {
        e.stopPropagation()

        if (isWatchListed) {
            props.setWatchList((prev) => {
                return prev.filter((movie) => movie.id !== props.id)
            })
        }
        else {
            props.setWatchList((prev) => {
    ...
    return (
        <>
            <div className="card" onClick={handleCardClick}>
                <button className="watchlist-btn" onClick={editWatchList} aria-label={isWatchListed ? "Remove from watchlist" : "Add to watchlist"}>
